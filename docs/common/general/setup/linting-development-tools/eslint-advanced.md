# ![eslint](../../_assets/icons/eslint-22.png) ESLint – Advanced
...
>       + [🚀 Use Flat Config (Modern ESLint Setup)](#-use-flat-config-modern-eslint-setup-)
>       + [🧱 Define with `defineConfig()](#-define-with-defineconfig-)
>       + [🎯 Combine `files` and `ignores` (Scoped Configuration)](#-combine-files-and-ignores-scoped-configuration-)
>       + [🚫 Ignore Files Globally](#-ignore-files-globally-)
>       + [🌐 Change Language Options](#-change-language-options-)
>       + [🌍 Specify Global Variables](#-specify-global-variables-)
>       + [🔎 Configure Parser](#-configure-parser-)
>       + [🧩 Register Plugins](#-register-plugins-)
>       + [🖇️ Extend Shared Configs](#-extend-shared-configs-)
>       + [📏 Setting Rules](#-setting-rules-)
>   - [📦 Popular Packages](#-popular-packages-)
>       + [🛠 Core & TypeScript](#-core--typescript-)
>       + [⚛️ Frameworks & Libraries](#-frameworks--libraries-)
>       + [📂 File Types](#-file-types-)
>       + [🎨 Style Guides (Shareable Configs)](#-style-guides-shareable-configs-)
>       + [🚀 Advanced / Utility](#-advanced--utility-)
...
## 🔧 How to Configure ESLint [🔺](#-eslint--advanced)

### 🚀 Use Flat Config (Modern ESLint Setup) [🔺](#-eslint--advanced)
...
> ### ⚖️ Flat Config vs. Legacy Config
>
...
### 🧱 Define with `defineConfig()` [🔺](#-eslint--advanced)
...
### 🎯 Combine `files` and `ignores` (Scoped Configuration) [🔺](#-eslint--advanced)
...
>
> **`basePath`:** _(optional)_ Specifies the root directory to resolve `files` and `ignores` from.
...
### 🚫 Ignore Files Globally [🔺](#-eslint--advanced)
...
export default defineConfig([
  includeIgnoreFile(gitignorePath, 'Import .gitignore patterns'),
]);
```

### 🌐 Change Language Options [🔺](#-eslint--advanced)
...
However, in some cases you may want to override these settings using the `languageOptions` property in a
**Config Object** — for example, when working with **legacy code** or **non-module environments**.

> ### 📘 _Key Fields_
>
> **`languageOptions.ecmaVersion`:** (_default: `latest`_) Defines **ECMAScript version** ESLint should understand.
>>   - `latest`         – the most recent supported version
>>   - A year           – _i.e. `2016`, `2020`_
>>   - A version number – _i.e. `5` (ES5), `6` (ES2015)_
>
> **`languageOptions.sourceType`:** (_default: `module`_) Determines how ESLint interprets your **module system**.
...
### 🌍 Specify Global Variables [🔺](#-eslint--advanced)
...
> ### 📘 _Key Fields_
>
> **`languageOptions.globals`:** Defines which **global variables** are available during linting and how they can be used in your code.
...
##### ❇️ Specify via Comment Directives:

You can also declare globals at the file level with the `/* global ... */` comment. This is useful when:
...
### 🔎 Configure Parser [🔺](#-eslint--advanced)

A **Parser** is responsible for converting source code into an **Abstract Syntax Tree (AST)**, which **ESLint** uses to
analyze and detect issues. By default, ESLint uses a parser designed for **JavaScript**, but you can configure
additional parsers to support other languages and syntaxes such as **TypeScript**, **JSX**, or even **CSS-in-JS**.

> ### 📘 _Key Fields_
>
> **`parser`:** Specifies which **Parser** to use for the current **Config Object**.
>
> **`parserOptions`:** Extra options passed to the parser (_**options** vary depending on the **Parser**_).

##### ❇️ JavaScript (Default) / TypeScript Parser:
...
You can customize parsing with the `parserOptions` field in a **Config Object**.
...
> ### 📕 _**Note**_
>
> When using a **Plugin**, you usually don’t need to configure the **Parser** explicitly — the **Plugin** takes care of
> it. Only override the **Parser** when you require **custom behavior** or **need to support non-standard syntax**.


### 🧩 Register Plugins [🔺](#-eslint--advanced)

**Plugins** are the primary way to extend **ESLint**’s functionality by **adding new rules**, **support for new
languages**, and enabling the linter to **enforce standards for specific frameworks** like **React**, **TypeScript**, or
**Angular** that are not covered by **ESLint**'s core features.
...
>>  **Syntax:** defined as an object mapping `{ <namespace>: <PluginObject> }`. `<namespace>` can be the same as the
>>              package name or a custom alias.\
...
>
> 🤘 _To use predefined configs, you can [apply them directly](#-apply-a-shared-config-directly) or [override them](#-overriding-settings-from-shared-configs-via-extends)._


### 🖇️ Extend Shared Configs [🔺](#-eslint--advanced)
...
Unlike [plugins](#-register-plugins), **configs** do not introduce new rules or functionality. Instead, they
package together **predefined rules**, **exiting plugins** and other settings into a reusable **ESLint config**. These
configs can include:
...
A **Shareable Config** is an ESLint config published as an **npm** package that you can install and reuse across
projects.
...
##### 🟢 Predefined Configs from Plugins:

Many **ESLint plugins** (e.g., **React**, **TypeScript**, **Vue**) ship with **predefined configs** that can be extended
directly. They automatically **activate the plugin** and **apply its rules**, so you don’t need to explicitly declare
the plugin in your config.
- **Purpose:** Make setup easier by activating both the **plugin** and its **recommended rules** in one step.
...
>>  - A configuration array – another array of configs to merge
>
> **Direct usage (without `extends`):** (_preferred_) You can also pass a configuration object directly to
> `defineConfig()`.
>>  - The config is applied **globally**.
>>  - Not intended for **inline overrides** or **file-specific** settings.

##### ❇️ Apply a Shared Config (directly):
...
##### ❇️ Overriding Settings from Shared Configs (via `extends`):
...
##### ❇️ Extending Configs from Plugins (via `extends`):
...
>>  **Example:** `no-console`, `@typescript-eslint/no-unused-vars`.
>
> **Severity levels:** Control how violations are reported.
>>  - `'off'` / `0`   — rule disabled
>>  - `'warn'` / `1`  — show a warning but doesn’t block builds
>>  - `'error'` / `2` — report an error and fail builds (_exits with non-zero exit code_)
>
> **With options:** Many rules accept an array where the first item is **severity** follow by that **rule options** to
> fine-tune behavior (_refer to the documentation for each rule_).
>> **Format:**
>>   - `[<severity-level>, '<string-option>']`
>>   - `[<severity-level>, { <object-option> }]`
>>   - `[<severity-level>, '<string-option>', { <object-option> }]`
>>
>> **Example:** `['error', 'always']`, `['error', 'always', { 'omitLastInOneLineBlock': true }]`

##### ❇️ Setting In Config File:

Define rules under the `rules` field of a **Config Object** inside `eslint.config.mjs` for project-wide linting.
...
##### ❇️ Setting via Comment Directives:

You can also configure rules inline in source files using ESLint comment directives, this applies only to that file.

📗 Example:

```js
/* eslint no-console: "off", no-unused-vars: "warn" -- Format: `<rule-name>: <severity-level>` */

/* eslint semi: ["error", "always"] -- Format: `<rule-name>: [<severity-level>, '<string-option>']` */

/* eslint quotes: ["error", "single", { "allowTemplateLiterals": true }] -- Format: `<rule-name>: [<severity-level>, '<string-option>', { <object-option> }]` */
```
...
### 🛠 Core & TypeScript [🔺](#-eslint--advanced)
...
### ⚛️ Frameworks & Libraries [🔺](#-eslint--advanced)
...
| `stylelint `                 | Separate linter for **CSS/SCSS/LESS** with in-depth style-specific rules | External tool | [Rules](https://stylelint.io/user-guide/rules) - [Installation](https://stylelint.io/user-guide/get-started)                                                     |


### 🎨 Style Guides (Shareable Configs) [🔺](#-eslint--advanced)
...
### 🚀 Advanced / Utility [🔺](#-eslint--advanced)
