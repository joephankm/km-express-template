# ![eslint](../_static/icons/eslint-28.png) ESLint
...
> ┣━ 🛠️ [Config File](eslint-advanced.md#-config-file-)\
> ┣━ 🧭 [Scope and Ignore Settings](eslint-advanced.md#-scope-and-ignore-settings-)\
> ┣━ 🔬 [Parsers](eslint-advanced.md#-parsers-)\
> ┣━ 🔌 [Plugins](eslint-advanced.md#-plugins-)\
> ┣━ 📤 [Shared Configs](eslint-advanced.md#-shared-configs-)\
...
### 📦 Install **ESLint** Core Packages

Add the **ESLint core package** and its **official base config**:

```shell
pnpm add -D eslint@latest @eslint/js@latest globals
...
### 🖇️ Install Plugins and Shareable Configs

Depending on your project stack, you may need additional plugins and shareable configs. A list of commonly used packages
is available in the [_Popular Packages_](eslint-advanced.md#-popular-packages-) section.

📗 **Example:** _Add prettier plugin to prevent formatting conflicts with Prettier_
...
After running the command, the CLI will guide you step by step through the setup process, prompting you to choose
options that match your project requirements.

👉 _See the [available options](https://eslint-airbnb-extended.nishargshah.dev/cli/guide#guide) and the [list of packages
   used](https://eslint-airbnb-extended.nishargshah.dev/config/packages-used) in the `create-airbnb-x-config` docs._
...
> concurrently 'pnpx eslint .' 'tsc --noEmit'
>
> # --- OR in `package.json` ---
> # "scripts": {
...
# --- OR in `package.json` ---
# "scripts": {
#   "lint:fix": "concurrently 'eslint .' 'tsc --noEmit'"
# }
```
...
> ### 🧠 _About the ESLint Config File_
>
...
>   - Use the `defineConfig()` helper from ESLint for better readability and type safety (**recommended**).
>
...
### ②. ![TypeScript](../_static/icons/typescript-16.png) Integrate with TypeScript (for TypeScript project)
...
##### ❇️ <u>Basic Integration (without Type Checking)</u>
...
Eslint**:
  ...
  2. _(Optional)_ Add stylistic rules for code formatting:
...
##### ❇️ <u>Enable Type-aware TypeScript Linting</u>
...
enable type-aware linting by the following steps:
  ...
  3. Disable type-aware linting for JavaScript files _(optional but **recommended**)_:
...
### ③. ![Prettier](../_static/icons/prettier-16.png) Integrate with Prettier (for projects using Prettier)
...
>
> 👉 _See [Formatters vs Linters in TypeScript ESLint](https://typescript-eslint.io/users/what-about-formatting#formatters-vs-linters) for a detailed comparison._
...
>
> 🤘 _For details on how plugins work internally, see [Plugins](eslint-advanced.md#-plugins-) guide._
...
> multiple projects.
>
> Unlike Plugins, Shareable Configs **do not introduce new rules**. Instead, they bundle together:
>   - **Preferred rules** (with predefined severity levels).
>   - References to **plugins** and other configs.
>   - Common **language or project settings**.
>
> You can obtain shareable configs by:
...
>
> 🤘 _See [Shared Configs](eslint-advanced.md#-shared-configs-) for details on combining configs._
...
> ### 🚧 Legacy Config Limitation
>
> The example above **will not work directly**, because `eslint-config-airbnb-base` is still a **Legacy Config**
> (`.eslintrc` file) and has not been ported to the new **Flat Config** format. Many popular style guide packages
> (including **Airbnb**) are still in this state.
>
> You have two options to proceed:
>
> ❇️ **Option 1** — _Use Legacy Config Inside Flat Config_
>
> ESLint provides a compatibility layer that **allows legacy eslintrc-based configs** to be used within Flat Config.
>
> 👉 _Refer to the official guild: [Using eslintrc Configs in Flat Config](https://eslint.org/docs/latest/use/configure/migration-guide#using-eslintrc-configs-in-flat-config)._
>
> ❇️ **Option 2** — _Use `eslint-config-airbnb-extended` Package_
>
> Use `eslint-config-airbnb-extended`, which is **Flat Config–ready** and designed as a modern replacement for Airbnb’s
> legacy configs. This setup is included and automatically configured when using `create-airbnb-x-config`.
>
> 👉 _See the [Install All-in-One section](#-install-all-in-one-with-create-airbnb-x-config) for details._
...
>
> 🤘 _See [Lint Rules](#-lint-rules-) for severity levels and options formats._
>
> ---
>
...
>
> 🤘 _See [Control Config Scope](eslint-advanced.md#-control-config-scope) for details on scoping configs._
...
📗 **Example:** _Centralize Project-wide Overrides_
...
📗 **Example:** _Specific Files or Folders Overrides_
...
#### 1. Global Variables (per environment):
...
(🤘 _See [Global Ignore](eslint-advanced.md#-ignore-files-globally) for more detail._)
...
To configure a rule, you assign it a **severity level**. This determines whether the rule is **disabled**, reported as a
**warning**, or enforced as an **error**.
...
>   - **Format:**  `<rule-name>` or `<namespace>/<rule-name>`.
>
> 📙 `<rule-severity>` — Control how violations are reported.
>   - `'off'` / `0`   – rule disabled.
>   - `'warn'` / `1`  – show a warning but doesn’t block builds.
>   - `'error'` / `2` – report an error and fail builds (_exits with non-zero exit code_).
...
Many rules support additional configuration. When options are provided, the rule value becomes an **array**, where the
first item defines the severity and the remaining items configure rule-specific behavior.

> ### 📘 Syntax
>
> ```
> <rule-name>: [<rule-severity>, '<string-option>']
> <rule-name>: [<rule-severity>, { <object-options> }]
> <rule-name>: [<rule-severity>, '<string-option>', { <object-options> }]
> ```
>
> 📙 `'<string-option>'` / `{ <object-options> }` — Rule-specific options.
>   - Available options vary by rule.\
>     (_Refer to each rule’s documentation for supported configuration._)
...
ESLint parses special comments that begin with `/* eslint ... */` to configure rules directly inside a file. These
comments let you **enable, disable, or adjust rules on a per-file**.

> ### 📘 Syntax
>
> ```js
> /* eslint <rule-name>: <rule-severity> [, <rule-name>: <rule-severity>, ...] [-- <description>] */
> ```
>
> 📙 `<rule-name>: <rule-severity>` — Rule config.\
> (🤘 _Refer to [Lint Rules](#-lint-rules-) for formats and severity levels._)
>
...
Your primary config should **always be in the config file**. Use file-level comments only when it’s necessary, such as:
  ...
  - Enabling an additional rule in a single file _(**not recommended** unless truly needed)_.
...
### ⚡️ TypeScript Directive Comments
...
- ⚠️ **Risky:** Removes all type-safety, so use sparingly.
...
### 🌍 Add Global Variables for a File

You can declare global variables at the file level using a `/* global ... */` block comment. This tells ESLint that
specific identifiers exist in the global scope for that file.
...
> ```
>
> 📙 `<var-name>` — The global variable name.
>
> 📙 `<value>` — _(optional, **default:** `readonly`)_ Access level.
>   - **Values:** `writable` or `readonly`.\
>     (🤘 _See [Specify Global Variables](eslint-advanced.md#-specify-global-variables) for value details._)

Useful when:
  + Using **external libraries** that inject variables into the global scope.
  + Defining **custom globals** that apply only to a single file (with **caution**).

📗 **Example:**

```js
/* global myGlobalVar1, myGlobalVar2:writable, readOnlyGlobal:readonly */

console.log(myGlobalVar1);   // -> ✅ No "no-undef" error
readOnlyGlobal = 42;         // -> ❌ Error, because it's readonly
```

------------


## ⚡️ Command Cheatsheet [🔺](#-eslint)

Here are the most commonly used ESLint CLI commands:

| Command                           | Description                                           | 📗 Example                           |
|-----------------------------------|-------------------------------------------------------|--------------------------------------|
| `eslint <file/dir>`               | Lint **all files** in place                           | `eslint src/`                        |
| `eslint <file/dir> --fix`         | Lint and **auto-fix** fixable issues in place         | `eslint src/ --fix`                  |
| `eslint <file/dir> --fix-dry-run` | Preview auto-fix changes **without writing** them     | `eslint src/ --fix-dry-run`          |
| `eslint --print-config <file>`    | Print the **resolved config** applied to a given file | `eslint --print-config src/index.ts` |
| `eslint --inspect-config`         | Open the **config inspector** in the browser          |                                      |
| `eslint --debug`                  | Enable **debug output** for troubleshooting           |                                      |

Common CLI options supported across ESLint commands:

| Options                | Description                                                              | Short Form | 📗 Example                            |
|------------------------|--------------------------------------------------------------------------|------------|---------------------------------------|
| `--quiet`              | Report **errors only**, suppress warnings                                |            |                                       |
| `--config <path>`      | Use a **specific config file** instead of the auto-detected one          | `-c`       | `eslint . --config eslint.config.mjs` |
| `--format <formatter>` | Use a **specific output formatter** (e.g., `stylish`, `json`, `compact`) | `-f`       | `eslint . --format json`              |

👉 _See the full list in the [ESLint CLI docs](https://eslint.org/docs/latest/use/command-line-interface)._
...
These files define how **ESLint** is configured for this project.
