# 🌿 JS/TS Environment

Documentation covering the tools, configuration, and setup used to establish a **consistent and reliable
JavaScript/TypeScript development environment**.

************

## 🧱 Node Core

Foundation tools for managing the **Node.js runtime** and **package manager** across projects and teams.

Overview: 🧱 [_Node Environment Foundation_](node-core/-intro.md)

- ![NVM](_static/icons/nvm-14.png) [NVM — Node version manager](node-core/nvm.md)
- ![Corepack](_static/icons/corepack-14.png) [Corepack — Package manager provisioning](node-core/corepack.md)
- ![PNPM](_static/icons/pnpm-14.png) [PNPM — Fast, disk-efficient package manager](node-core/pnpm.md)
- ![Yarn](_static/icons/yarn-14.png) [Yarn — Modern package manager](node-core/yarn.md)

------------


## 🏗️ Build Tools

Tools responsible for **transforming**, **bundling**, and **preparing source code** for execution or deployment.

Overview: 🏗️ [_Compilers & Build Tools_](compilers/-intro.md)

- 🛡️ [TSC — TypeScript Compiler](compilers/tsc.md)

------------


## 🧰 Environment Tools

Runtime and environment utilities that support development and execution of TypeScript applications.

Overview: 🧰 [_Environment Tools_](tools/-intro.md)

- ![Dotenv CLI](_static/icons/dotenv-14.png) [Dotenv CLI — Load `.env` files from the command line](tools/dotenv-cli.md)

------------


## 🛠️ Setup

Step-by-step guides for configuring the full development environment from scratch.

Overview: 🌳 [_Environment Setup Guides_](setup/-intro.md)

- 🧭 [Manage Node Runtime & Package Manager Versions](setup/node-package-manager-version.md)
- ![TypeScript](_static/icons/typescript-14.png) [Configure TypeScript Environment](setup/typescript-environment.md)
- 🌳 [Setup Environment in JavaScript Application](setup/javascript-environment.md)

------------


## 🧠 Fundamentals

Core concepts behind how JavaScript organizes and executes code across different environments.

Overview: 🌳 [_Environment Fundamentals_](fundamentals/-intro.md)

- 🧩 [JavaScript Module Systems](fundamentals/module-systems.md)
